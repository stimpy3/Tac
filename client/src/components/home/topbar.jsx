  ...
  return (
    <div className="w-full h-[100px] pl-[20px] flex justify-between items-center">
      ...
      <div className="flex flex-col justify-center">
        <h1 className="text-[1.5rem] text-accentTxt">Welcome {username.split(" ")[0]} 👋</h1>
        <p className="text-[1rem] text-gray-500">
          <span className="text-accentS3 text-[0.9rem]">{date}</span>
        </p>
      </div>

      {/* Profile and top icons */}
      <div className="h-full flex items-center gap-[20px]">
        <div className="min-w-[70px] flex justify-between items-center h-full text-accentS3">
          <button><Sun /></button>
          <button><i className="fa-solid fa-bell text-[1.3rem]"></i></button>
        </div>

        <div className="h-[60px] w-[220px] bg-accentM flex items-center rounded-full pl-[5px] border border-accentBorder2">
          {
            (user && user?.picture && !user.picture.includes("default-user")) ? (
              <div className="w-[50px] h-[50px] rounded-full border border-accentBorder2 shadow-lg overflow-hidden">
                <img
                  src={user.picture}
                  alt="User"
              ...
              <div className={`flex items-center justify-center text-[1.4rem] text-white w-[50px] h-[50px] ${userColor} rounded-full border border-accentS3 shadow-lg`}>
          ...
          <section className="w-[150px] h-full flex justify-center items-center">
            <section className="pl-[5px] w-full h-full flex flex-col justify-center items-start overflow-hidden">
              <p className="text-[0.9rem] mb-[2px] text-accentTxt">
                {(username.length > 15) ? username.slice(0, 10) + "..." : username}
              </p>
              <p className="text-accentS3 text-[0.6rem]">{useremail}</p>
            </section>
